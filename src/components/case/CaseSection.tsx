...
export function CaseSection({
  ...
  const id = `case-${title.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
  return (
    <section
      data-theme={theme}
      data-index={n}
      data-label={title}
      aria-labelledby={id}
      className={cn("relative px-gutter py-[12vh]", className)}
    >
      <div className="grid gap-8 border-t border-line pt-6 lg:grid-cols-12 lg:gap-6">
        <div className="lg:col-span-3">
          <h2 id={id} className="label flex gap-3 lg:sticky lg:top-[calc(var(--nav-h)+20px)]">
